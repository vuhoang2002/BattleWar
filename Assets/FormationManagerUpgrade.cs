...
public class UnitInfo
{
    ...
    private void ArrangeFormation(){
        ...
        for (int i=1; i<=unitInfo_List.Count;i++){// tối đa 6
        ...
        for(int col=1;col<=totalColum;col++){ //xét các cột
         Debug.Log(preX+","+ preY+","+ minY);
            ...
            for(int i=0; i< unitPerCol[col-1] ;i++){// thiết lập tọa độ vị trị 
    ...
    private void UpdateUnitList_Info(bool add_Or_Del, string unitName){
        ...
        if(!flag){//unitl chưa có trong danh sách(chưa có tức là phải add)
